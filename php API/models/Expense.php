...
class Expense {
    ...
    public function delete() {
        // Obtener los detalles del gasto antes de eliminarlo
        ...
        if ($stmt->rowCount() > 0) {
            ...
            if ($stmt->execute()) {
                ...
                if ($paymentMethod->update()) {
                    ...
                    $log = new Log($this->conn);
                    $log->month = date('Y-m');
                    $log->movement_type = "Gasto Eliminado";
                    $log->movement_id = $this->id;
                    $log->details = json_encode([
                        "cantidad" => $this->amount,
                        "id_payment" => $this->id_payment,
                ...
                // Manejo de error en caso de que no se pueda eliminar el gasto
                return false;
            }
        } else {
            // Manejo de error en caso de que no se pueda encontrar el gasto
            return false;
        }
    }
}
